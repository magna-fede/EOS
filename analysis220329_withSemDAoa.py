...
to_norm = stimuliALL[['ConcM','LEN','UN2_F','UN3_F','Orth','OLD20','FreqCount','LogFreq(Zipf)', 
                     ...
                     'LENprec','Predictability','cloze','plausibility','Sim','SemD','AoA']]
...
for dat,name in zip([dur,
    ...
    with open(f"U:/AnEyeOnSemantics/41analysis/{name}_withSemDAoA.P", 'wb') as outfile:
        pickle.dump(participants,outfile)  

pd.concat(norm_ffd_all).to_csv('C:/Users/fm02/OwnCloud/EOS_EyeTrackingDataCollection/Data_Results/data_forR/norm_ffd_41_withSemDAoA.csv',index=False)

pd.concat(norm_gd_all).to_csv('C:/Users/fm02/OwnCloud/EOS_EyeTrackingDataCollection/Data_Results/data_forR/norm_gd_41_withSemDAoA.csv',index=False)
